    ...
    for wav_path in tqdm(all_wav_files, desc="Processing WAV files"):
        ...
        if is_augmented:
            ...
            if original_filename in filename_to_metadata:
                ...
                path_for_manifest = os.path.join(
                    "pd_dataset", relative_path).replace(
                    "\\", "/")
                ...
                print(
                    f"Warning: Found augmented file without original: {filename}")
        else:
            ...
            # Try to extract from filename (common format:
            # B1LBULCAAS94M100120171021)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create comprehensive manifest including all WAV files")
    parser.add_argument(
        "--original_manifest",
        required=True,
        help="Path to the original manifest JSON file")
    parser.add_argument(
        "--data_dir",
        required=True,
    ...
    parser.add_argument(
        "--output_manifest",
        required=True,
        help="Path to save the new manifest")
    parser.add_argument(
        "--include_augmented",
        action="store_true",
        help="Include augmented files in the manifest")
