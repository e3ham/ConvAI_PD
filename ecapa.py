...
class ParkinsonsBrain(sb.Brain):
    ...
    def on_stage_start(self, stage, epoch=None):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if not hasattr(self, "all_ids"):
                ...
                print(f"Using existing tracking lists with {len(self.all_ids)} items for {stage}")
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = sb.nnet.losses.nll_loss(torch.log_softmax(predictions, dim=-1), labels)
        ...
        if stage != sb.Stage.TRAIN:
            ...
            for i in range(len(pred_indices)):
                pred = pred_indices[i].item() if pred_indices[i].numel() == 1 else pred_indices[i][0].item()
                target = labels[i].item() if labels[i].numel() == 1 else labels[i][0].item()
    ...
    def audio_pipeline(path):
        ...
        if "Italian_Parkinsons_Voice_and_Speech" in path or "italian_parkinson" in path:
            ...
            try:
                ...
                group_index = next(i for i, part in enumerate(parts) if group in part)
...
# if evaluate returned a dict, extract its 'error', else assume it's the float itself
...
if hasattr(pd_brain, "all_preds") and hasattr(pd_brain, "all_targets"):
    ...
    y_pred = np.array([p.item() if torch.is_tensor(p) else p for p in pd_brain.all_preds])
    y_true = np.array([t.item() if torch.is_tensor(t) else t for t in pd_brain.all_targets])
