...
def separate_tasks(data_dir, output_base_dir, manifests_dir=None, use_symlinks=True):
    ...
    print(f"  Total: {stats['reading']['original'] + stats['reading']['augmented']}")
    
    print("\nOther task files:")
    print(f"  Original: {stats['other']['original']}")
    print(f"  Augmented: {stats['other']['augmented']}")
    print(f"  Total: {stats['other']['original'] + stats['other']['augmented']}")
    
    # Create manifests for each task if manifest directory is provided
    if manifests_dir and os.path.exists(manifests_dir):
        create_task_manifests(manifests_dir, data_dir, reading_task_dir, other_task_dir, output_base_dir)
    
    return reading_task_dir, other_task_dir

def create_task_manifests(manifests_dir, original_data_dir, reading_task_dir, other_task_dir, output_base_dir):
    """
    ...
    for split in ["train", "valid", "test"]:
        ...
        for key, info in tqdm(original_manifest.items(), desc=f"Processing {split} manifest"):
            ...
            if is_reading:
                # Create updated info with path pointing to reading task directory
                updated_info = dict(info)
                
                # Adjust the path if needed
                if not os.path.exists(file_path) and original_data_dir in file_path:
                    # If original path is absolute and contains the original data dir
                    rel_path = os.path.relpath(file_path, original_data_dir)
                    updated_info["path"] = os.path.join(reading_task_dir, rel_path)
                elif not os.path.exists(file_path) and not os.path.isabs(file_path):
                    # If original path is relative
                    updated_info["path"] = os.path.join(reading_task_dir, file_path)
                
                # Add to reading manifest
                reading_manifest[key] = updated_info
            else:
                # Create updated info with path pointing to other task directory
                updated_info = dict(info)
                ...
                if not os.path.exists(file_path) and original_data_dir in file_path:
                    # If original path is absolute and contains the original data dir
                    rel_path = os.path.relpath(file_path, original_data_dir)
                    updated_info["path"] = os.path.join(other_task_dir, rel_path)
                elif not os.path.exists(file_path) and not os.path.isabs(file_path):
                    # If original path is relative
                    updated_info["path"] = os.path.join(other_task_dir, file_path)
                
                # Add to other manifest
                other_manifest[key] = updated_info
        
        # Save reading manifest
        reading_manifest_path = os.path.join(reading_manifests_dir, f"{split}.json")
        with open(reading_manifest_path, 'w') as f:
            json.dump(reading_manifest, f, indent=2)
        
        # Save other manifest
        other_manifest_path = os.path.join(other_manifests_dir, f"{split}.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Separate audio files into different tasks")
    
    parser.add_argument("--data_dir", required=True, help="Root directory of the original dataset")
    parser.add_argument("--output_dir", required=True, help="Base directory where separated task folders will be created")
    parser.add_argument("--manifests_dir", help="Directory containing manifest files (optional)")
    parser.add_argument("--augmented_dir", help="Directory containing augmented files (optional)")
    ...
    if args.augmented_dir and os.path.exists(args.augmented_dir):
        scan_augmented_files(args.data_dir, args.augmented_dir, args.output_dir)
