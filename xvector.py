    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            for i in range(len(pred_indices)):
                pred = pred_indices[i].item() if isinstance(pred_indices[i], torch.Tensor) else pred_indices[i]
                target = labels[i].item() if isinstance(labels[i], torch.Tensor) else labels[i]
