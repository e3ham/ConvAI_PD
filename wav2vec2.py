...
# Brain class for Parkinson detection - Enhanced with wav2vec2 features
...
if __name__ == "__main__":
    ...
    # Set up the wav2vec2 model
    if "wav2vec2" in hparams["modules"]:
        hparams["modules"]["wav2vec2"].to(run_opts["device"])
        
        # Freeze feature extractor if specified
        if not hparams["freeze_ssl"] and hparams.get("freeze_ssl_conv", False):
            hparams["modules"]["wav2vec2"].model.feature_extractor._freeze_parameters()
